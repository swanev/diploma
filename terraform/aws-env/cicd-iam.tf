...
resource "aws_iam_role_policy_attachment" "codebuild-to-eks-policy" {
  policy_arn = "arn:aws:iam::aws:policy/AmazonEKSClusterPolicy"
  role       = aws_iam_role.tf-codebuild-role.name
}

resource "aws_iam_role_policy_attachment" "codebuild-to-eks-controller" {
...
data "aws_iam_policy_document" "tf-cicd-build-policies" {
    statement{
        sid = ""
        actions = ["logs:*", "s3:*", "codebuild:*", "secretsmanager:*","iam:*", "ecr:BatchCheckLayerAvailability", "ecr:CompleteLayerUpload",
